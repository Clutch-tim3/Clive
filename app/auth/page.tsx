...
const GitLabSVG = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="#FC6D26">
...
function OAuthRow({ onOAuth }: { onOAuth?: (provider: 'google' | 'github') => void }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '22px' }}>
      {[
        ...
        { label: 'GitLab', icon: <GitLabSVG />, key: null },
      ].map(({ label, icon, key }) => (
        <button key={label} className="auth-ob" onClick={() => key && onOAuth?.(key)}>
          {icon}{label}
        </button>
      ))}
    </div>
...
function AuthContent() {
  ...
  const handleSuccess = () => {
    setVisible(false);
    setTimeout(() => { setScreen('onboard'); setVisible(true); }, 165);
  };
